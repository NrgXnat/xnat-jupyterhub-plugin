...
XNAT.plugin.jobTemplates.computeSpecConfigs = getObject(XNAT.plugin.jobTemplates.computeSpecConfigs || {});
...
(function (factory) {
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.get = async (id) => {
        console.debug("Fetching compute spec config " + id)
        const url = XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs/${id}`);
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.getAll = async (type) => {
        console.debug("Fetching compute spec configs")
        
        const url = type ?
            XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs?type=${type}`) :
            XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs`);
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.create = async (computeSpec) => {
        console.debug("Creating compute spec config")
        const url = XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs`);
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.update = async (computeSpec) => {
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.save = async (computeSpec) => {
        console.debug("Saving compute spec config")
        const id = computeSpec['id'];
        
        if (id) {
            return XNAT.plugin.jobTemplates.computeSpecConfigs.update(computeSpec);
        } else {
            return XNAT.plugin.jobTemplates.computeSpecConfigs.create(computeSpec);
        }
    }
    
    XNAT.plugin.jobTemplates.computeSpecConfigs.delete = async (id) => {
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.available = async (type, user, project) => {
        console.debug(`Fetching available compute spec configs for type ${type} and user ${user} and project ${project}`);
        const url = type ?
            XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs/available?type=${type}&user=${user}&project=${project}`) :
            XNAT.url.csrfUrl(`/xapi/job-templates/compute-spec-configs/available?user=${user}&project=${project}`);
    ...
    XNAT.plugin.jobTemplates.computeSpecConfigs.manager = async (containerId, computeSpecType) => {
        ...
        const getHardwareConfigs = () => {
            XNAT.plugin.jobTemplates.hardwareConfigs.getAll().then(h => hardwareConfigs = h);
        }
        ...
        const enabledToggle = (config) => {
            ...
            let ckbox = spawn('input', {
                ...
                onchange: () => {
                    ...
                    XNAT.plugin.jobTemplates.computeSpecConfigs.update(config).then(() => {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        action: function() {
                            ...
                            XNAT.plugin.jobTemplates.computeSpecConfigs.save(config)
                                .then(() => {
                                    refreshTable();
                                    XNAT.dialog.closeAll();
                                    XNAT.ui.banner.top(2000, 'Save', 'success');
                                })
                                .catch((err) => {
                                    console.error(err);
        ...
        const refreshTable = () => {
            XNAT.plugin.jobTemplates.computeSpecConfigs.getAll(computeSpecType)
                .then((data) => {
        ...
        const deleteConfig = (id) => {
            XNAT.plugin.jobTemplates.computeSpecConfigs.delete(id)
                .then(() => {
