...
- [XNAT-8763]: Upgrade the build to Java 21, Gradle 9.4.1, and the XNAT 1.10.1-SNAPSHOT monorepo BOM. Test fixes for
               the BOM-driven dependency bumps (H2 2.x, Jackson, log4j). CI builds now use the Java/Gradle defaults
               from the `build-plugin-jar` action.
...
- [XNAT-8655]: Add more info to UI when linking/copying shared data. Remove shared data directory in background and
               stop Jupyter notebook server first.

## [1.3.3] - 2025-12-03

No changes to the plugin jar, only the Docker images and Helm chart have been updated.
